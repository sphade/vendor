...
import { usePhoneNumber } from "../hooks/queries";
...
const GetHelp = () => {
  ...
  const onSubmit = (data: any) =>
    getHelp.mutate({ ...data, phone: `+${data?.phone}` });
  return (
      ...
      <div className="w-[520px]  border border-[#BDBDBD] px-[75px] bg-secondary rounded-lg ">
        ...
        <div className="flex items-center mb-8 gap-5">
          ...
          <p className="text-gray-500 font-semibold flex">
            mobile:{" "}
            <span className=" text-tertiary">
              {formatPhoneNumber(number?.data?.[0]?.phone)}
            </span>
          </p>
        </div>
        ...
        <div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="mb-20 space-y-5">
            <TextField
              fullWidth
              label="full Name"
              type={"text"}
            ...
            <Button loading={getHelp.isLoading} full>
              send
            </Button>
          </form>
        </div>
